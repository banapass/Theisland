...
public class Flaregun : MonoBehaviour {
	...
	void Update () 
	{
		...
		if(Input.GetButtonDown("Fire1") && !GetComponent<Animation>().isPlaying && ActionController.currentEquip == "Flare Gun")
		{
			if(currentRound > 0)
            {
				Shoot();
				SceneMove.instance.Invoke("EndingScene", 3);
			}
		...
		if(Input.GetKeyDown(KeyCode.R) && !GetComponent<Animation>().isPlaying && ActionController.currentEquip == "Flare Gun")
			Reload();
